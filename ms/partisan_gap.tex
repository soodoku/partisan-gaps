...
\begin{table}[t] \centering \normalsize \setlength\tabcolsep{10 pt} \setlength{\defaultaddspace}{0pt}
	\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}
	...
	\begin{adjustbox}{max width=\textwidth}
		...
		\end{tabular}
	\end{adjustbox}
	\caption*{\footnotesize Dependent variables are indicators for whether the individual responded that unemployment or the budget deficit has gone up since the 2010 midterm elections.
		Rep. cue is an indicator for whether the question stem includes "when Republicans gained control of the US Congress".
		The baseline is a Democratic cue for the question stem "when Democrats retained control of the Senate".
...
\cref{tab:partisangaps-yougov} reports the YouGov survey results from estimating \cref{eq:partisangap-yougov} to assess how manipulated partisan cues in the question stem affect partisan responses to questions about unemployment and the budget deficit.
As expected, Republicans are much more likely than the Democrats to say that unemployment has gone up. Compared to Democrats, Republics are 25 percentage points more likely to say that unemployment has gone since the 2010 midterm elections when the question cues a Democratic blame using the stem "when Democrats retained control of the Senate" ($p<0.001$). 
Presumably this is a consequence of the question stem holding the president (a Democrat) responsible for unemployment rate and budget deficit.
When the question instead cues a Republican blame using the stem "when Republicans regained control of the US Congress," Republicans become 29 percentage points less likely to say that unemployment has gone up. 
...
For the question about whether the budget deficit has gone up, the partisan responses and how they behave with the manipulated partisan cues in the question stem is qualitatively similar to the question about unemployment.
Republicans are more likely than Democrats to say that the deficit has gone up with a Democratic cue (34 percentage points more likely, $p<0.001$), but this behavior completely reverses when the cue places the blame on Republicans.
\clearpage
