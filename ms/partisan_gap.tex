...
In this paper, we extend the investigation into the role of survey design in explaining partisan differences. We report results from a new set of experiments that manipulate common features of frequently used survey items. The features we focus on plausibly encourage people to guess when they don't know or report attitudes instead of knowledge, and therby encourage partisan inference. We also assess what difference a items that not only assess knowledge but also confidence in that knowledge make. Do partisan gaps persist if only responses that participants are confident in are coded as correct?
