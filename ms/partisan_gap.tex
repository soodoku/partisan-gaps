...
% TODO guesses only cause structured errors if they are non random. Should we emphasize this more here?
...
\begin{center}
	\begin{figure}[t]
		...
		\caption*{\footnotesize 
			...
			Columns indicate the five different conditions described in \Cref{tab:conditions}. Rows indicate the nine individual survey question items described in \cref{si:mturk} plus their average.
...
% TODO: Do we want to keep this? should we do an analysis that looks at the effect of all treatment arms? I think this needs to be removed or put into the second section of the MS. It does not fit here.

In a second study we examine how congenial partisanship affects answers to survey questions in multiple choice and Likert scale response options. In this study we code answers by the respondents to survey questions as correct (when they select the correct response in the multiple choice arm or indicate full confidence with the correct response in the Likert arm). Congenial partisanship is coded for as 1 for instances where the individuals partisanship aligns with the content of the question and 0 otherwise. For example, the question about Donald Trump's executive order is coded as congenial for Republican survey participants but as uncongenial for Democrats.
