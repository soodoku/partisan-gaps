...
twoway 	
		...
		(rcap uci lci porder, 
			...
			ytitle("Predicted proportion of correct" "response by condition", size(vlarge))
		)		
	;
#delimit cr	
...
reg fedtaxdk i.($congguess), vce(hc3)
...
twoway 	
		...
		(rcap uci lci porder, 
			...
			bgcolor(white)
			plotregion(margin(10 10 0 0))
			ytitle("Predicted proportion of DK" "response by condition", size(vlarge))
		)		
	;
#delimit cr	
graph export $figsavedir/texas-fedtax-answerdk-congenialcue.pdf, replace	
