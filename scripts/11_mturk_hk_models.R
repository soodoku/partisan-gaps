...
screenreg(list(lm_mc10_c, lm_mc7_c),
          omit.coef = "questions",
          custom.model.names =  c("MC10", "MC 7"))
...
table(mturk_hk_closed_correct$response_probe[!is.na(mturk_hk_closed_correct$congenial)])
table(mturk_hk_probes$knowledge[!is.na(mturk_hk_probes$congenial)])
#table(mturk_hk_probes$cheating)
#table(mturk_hk_probes$guessing)
#table(mturk_hk_probes$inference)
#table(mturk_hk_probes$expressive)



# TODO: recode the knowledge probe coding, 0s from incorrect answers remain 

lm_mcprobe   <- lm_robust(response_probe ~ congenial + questions, data = subset(mturk_hk_closed_correct, probe == "closed"), clusters = respondent, se_type = "stata")
